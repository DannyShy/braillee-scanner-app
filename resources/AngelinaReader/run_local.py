...
parser.add_argument('model_path', type=str, help='Path to the model weights file (model.t7)')
...
if not Path(args.model_path).exists():
    print('model file does not exist: ' + args.model_path)
    exit()

recognizer = infer_retinanet.BrailleInference(
    params_fn=os.path.join(local_config.data_path, 'weights', 'param.txt'),
    model_weights_fn=os.path.join(args.model_path),
    create_script=None)
